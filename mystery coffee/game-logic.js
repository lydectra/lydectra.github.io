AFRAME.registerComponent('game-logic', {
    ...
    tick: function () {
        ...
        if (Math.abs(rot.y) % 360 < 15 || Math.abs(rot.y) % 360 > 345) {
            checkE = E; checkW = W; checkN = N; checkS = S; dir = 2;
        }

        if (Math.abs(rot.y + 90) % 360 < 15 || Math.abs(rot.y + 90) % 360 > 345) {
            checkN = E; checkS = W; checkW = N; checkE = S; dir = 0;
        }

        if (Math.abs(rot.y - 90) % 360 < 15 || Math.abs(rot.y - 90) % 360 > 345) {
            checkS = E; checkN = W; checkE = N; checkW = S; dir = 1;
        }

        if (Math.abs(rot.y + 180) % 360 < 15 || Math.abs(rot.y + 180) % 360 > 345) {
            checkW = E; checkE = W; checkS = N; checkN = S; dir = 3;
        }
    ...
    transportMove: function (d) {
        switch (d) {
            case 0:
                this.loadScene(MAP[this.yi][this.xi + 1]);
                break;
            case 1:
                this.loadScene(MAP[this.yi][this.xi - 1]);
                break;
            case 2:
                this.loadScene(MAP[this.yi - 1][this.xi]);
                break;
            default:
                this.loadScene(MAP[this.yi + 1][this.xi]);
        }
    ...
    loadScene: function (s) {
        ...
        setTimeout(() => {
            document.querySelector('#scene' + s).removeAttribute('animation');
        }, 1000);
        ...
        setTimeout(() => {
            document.querySelector('#sky').removeAttribute('animation');
            this.timer = 250;
        }, 1000);
    },
